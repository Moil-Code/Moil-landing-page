...
describe('preview creative contrast', () => {
	...
	it('preview action orange uses accessible ink and active chips keep their fill', () => {
		...
		assert.match(css, /\.preview-primary-cta\s*\{[^}]*color:\s*#0D091C[^}]*background:\s*var\(--orange\)/s);
	});
});
