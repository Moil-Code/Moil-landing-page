...
export const TYPEOUT_FLUSH_MS_PER_CHAR: number;
export const READY_HOLD_MAX_MS: number;
export const REVEAL_STEP_MS: number;
...
export const COMPOSE_MAX: number;
export const WAIT_COMPOSE_MAX: number;
export function asText(value: unknown): string;
export function asList(value: unknown): string[];
export function httpsUrl(raw: unknown): string;
export function stripMarkdown(raw?: unknown): string;
export function composeProse(raw?: unknown, max?: number): string;
export function languageLabel(raw?: unknown): string;
...
export function visibleBeatCount(
	beats?: Array<{ heading: string; text: string }> | null,
	typedChars?: Record<string, number> | null,
