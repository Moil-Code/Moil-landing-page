 ...
 * because the same script also runs on hosts whose sitemap lists PRODUCTION
 * URLs without being production — only the host that actually serves
 * www.moilapp.com may announce a change. THIS repo is that host, so set
 * INDEXNOW_SUBMIT=1 here (see DEPLOYMENT.md).
 */
